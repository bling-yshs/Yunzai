...
class PluginsLoader {
  ...
  reply(e) {
     ...
     * @param data.replyTo 指定回复的消息ID（可选，不传则回复触发消息）
     */
    e.reply = async (msg = "", quote = false, data = {}) => {
      if (!msg) return false

      let { recallMsg = 0, at = "", replyTo = null } = data
      ...
      // 支持指定回复的消息ID，如果传入 replyTo 则使用它，否则使用触发消息的 message_id
      const replyMessageId = replyTo || e.message_id
      if (quote && replyMessageId) {
        if (Array.isArray(msg)) msg.unshift(segment.reply(replyMessageId))
        else msg = [segment.reply(replyMessageId), msg]
      }
